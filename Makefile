# This Makefile is for the Dancer::Plugin::EmptyGIF extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Dancer/Plugin/EmptyGIF.pm]
#     AUTHOR => [q[David Moreno <david@axiombox.com]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[perl]
#     NAME => q[Dancer::Plugin::EmptyGIF]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], lib::abs=>q[0] }
#     VERSION_FROM => q[lib/Dancer/Plugin/EmptyGIF.pm]
#     clean => { FILES=>q[Dancer-Plugin-EmptyGIF-*] }
...
NAME_SYM = Dancer_Plugin_EmptyGIF
VERSION = 0.1
...
MAKE = make
FULLEXT = Dancer/Plugin/EmptyGIF
BASEEXT = EmptyGIF
PARENT_NAME = Dancer::Plugin
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Dancer/Plugin/EmptyGIF.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Dancer/Plugin
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Dancer/Plugin
...
TO_INST_PM = lib/Dancer/Plugin/EmptyGIF.pm

PM_TO_BLIB = lib/Dancer/Plugin/EmptyGIF.pm \
	blib/lib/Dancer/Plugin/EmptyGIF.pm
...
DISTNAME = Dancer-Plugin-EmptyGIF
DISTVNAME = Dancer-Plugin-EmptyGIF-0.1
...
manifypods : pure_all  \
	lib/Dancer/Plugin/EmptyGIF.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Dancer/Plugin/EmptyGIF.pm $(INST_MAN3DIR)/Dancer::Plugin::EmptyGIF.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Quick empty GIF response'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''David Moreno <david@axiombox.com'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Dancer-Plugin-EmptyGIF' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  lib::abs: 0' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: 0.1' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Quick empty GIF response",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "David Moreno <david@axiombox.com"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Dancer-Plugin-EmptyGIF",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "lib::abs" : 0' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Quick empty GIF response</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>David Moreno &lt;david@axiombox.com</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="lib::abs" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Dancer/Plugin/EmptyGIF.pm blib/lib/Dancer/Plugin/EmptyGIF.pm 
